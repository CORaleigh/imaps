  ...
  view.when(async () => {
    ...
    if (url.searchParams.get('app')) {
      ...
      if (config === 'puma') {
        ...
        if (!vpn) {
          sendAlert(
            'To view utility layers, you must be connected to the city network or connected through VPN. You will be redirected to the public iMAPS.',
            'Not on City Network or VPN.',
            setAlert,
            setLoading,
          );
          setTimeout(() => {
