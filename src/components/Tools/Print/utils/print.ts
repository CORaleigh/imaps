...
import { printTemplates, Template } from "./templates";
...
export type Format = 'pdf' | 'png32' | 'png8' | 'jpg' | 'gif' | 'eps' | 'svg' | 'svgz';
...
export const getLayouts = async (): Promise<Layout[]> => {
  ...
  printTemplates.results[0].value.forEach((value: Template) => {
...
//let mapViewStationary: __esri.WatchHandle;
...
export const showPrintFrame = (
  ...
  if (printTemplate) {
    addPrintGraphic(view, printTemplate, printScale, imageHeight, imageWidth, selectedTab);
...
function isPrintResponse(value: unknown): value is __esri.PrintResponse {
